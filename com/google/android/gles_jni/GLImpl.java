    ...
    implements GL10, GL10Ext, GL11, GL11Ext, GL11ExtensionPack
{
    ...
    private static boolean allowIndirectBuffers(String s)
    {
        ...
        flag = false;
        flag1 = false;
        obj = AppGlobals.getPackageManager();
        obj = ((IPackageManager) (obj)).getApplicationInfo(s, 0, UserHandle.myUserId());
        int i = ((flag1) ? 1 : 0);
        if(obj != null)
            try
            {
                i = ((ApplicationInfo) (obj)).targetSdkVersion;
            }
            catch(RemoteException remoteexception)
            {
                i = ((flag1) ? 1 : 0);
            }
        Log.e("OpenGLES", String.format("Application %s (SDK target %d) called a GL11 Pointer method with an indirect Buffer.", new Object[] {
            s, Integer.valueOf(i)
        }));
        if(i <= 3)
            flag = true;
