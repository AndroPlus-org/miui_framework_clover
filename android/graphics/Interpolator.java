...
public class Interpolator
{
    ...
    private static native void nativeReset(long l, int i, int j);

    private static native void nativeSetKeyFrame(long l, int i, int j, float af[], float af1[]);

    private static native void nativeSetRepeatMirror(long l, float f, boolean flag);

    private static native int nativeTimeToValues(long l, int i, float af[]);
    ...
    public Result timeToValues(int i, float af[])
    {
        if(af != null && af.length < mValueCount)
            throw new ArrayStoreException();
        switch(nativeTimeToValues(native_instance, i, af))
        {
        default:
